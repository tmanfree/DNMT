...
####################
    ...
    # Name: snmp_get_switch_macs
    # Input:
    #   ipaddr (string)
    #      -The ipaddress/hostname to grab info from
    # Return:
    #  a data structure with just the mac addresses(a list of strings of the interface descriptions)
    # Summary:
    #   grabs information from the switch to put in a datastructure

    def snmp_get_switch_macs(self, ipaddr, **kwargs):
        ...
        vendor = self.snmp_get_vendor_string(ipaddr, ro=kwargs.get('ro'))
        test = self.snmp_get_mac_table_bulk(ipaddr,vendor,ro=kwargs.get('ro'))
        for vlan in switchStruct.vlanList:
            vlan
        if vendor != "None Found":
            ...
            #     if port is not None and switchStruct.getSwitch(
            #             port['Switch']) is not None and switchStruct.getSwitch(
            #             port['Switch']).id is not None:  # added catch for provisioned switch 1 being missing
            ...
            #         poe_offset = 0
            #         if "WS-C3560-8PC" in switchStruct.getSwitch(port[
            #                                                         'Switch']).model:  # fix for WS-C3560-8PC switches being offset by 1 for poe (poe 2 is port 1, etc)
            #             poe_offset = 1
            #         elif "WS-C3560-24P" in switchStruct.getSwitch(
            #                 port['Switch']).model or "WS-C3560-48P" in switchStruct.getSwitch(
            #                 port['Switch']).model or "C3560V2" in switchStruct.getSwitch(port['Switch']).model:
            #             poe_offset = 2
            #         switchport = switchStruct.getSwitch(port['Switch']).getModule(0).getPort(
            #             port['Port'] - poe_offset)
            ...
            #             foundport.datavlanname = next((vlanEntry['Name'] for vlanEntry in switchStruct.vlanList if
            #                                            vlanEntry["ID"] == port['Vlan']), None)
            ...
            #             foundport.historicalinputerrors.append(
            #                 (int(datetime.datetime.now().strftime("%Y%m%d%H%M")), port['Errors']))
            #
            ...
            #             foundport.historicaloutputerrors.append(
            #                 (int(datetime.datetime.now().strftime("%Y%m%d%H%M")), port['Errors']))
            #
            ...
            #             foundport.historicalinputcounters.append(
            #                 (int(datetime.datetime.now().strftime("%Y%m%d%H%M")), port['Counters']))
            #
            ...
            #             foundport.historicaloutputcounters.append(
            #                 (int(datetime.datetime.now().strftime("%Y%m%d%H%M")), port['Counters']))
            #
